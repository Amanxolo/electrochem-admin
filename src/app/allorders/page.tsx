...
interface IPayment { _id: string; amount: number; payment_mode: string; payment_status: string; }
interface IUser { _id: string; name: string; email: string; userType: "reseller" | "oem" | "individual"; }
interface IItems { product_id: { _id: string; productName: string; productCategory: string; }; quantity: number; Price: number; }
interface IOrder { _id: string; user: IUser; totalAmount: number; status: "pending" | "placed" | "processing" | "shipped" | "delivered" | "cancelled"; createdAt: string; payment: IPayment; shippingAddress: Address; items: IItems[]; }
...
export default function AllOrdersPage() {
  ...
  const fetchOrders = async (idSearch?: string) => {
    try {
      ...
      if (res.ok) {
        ...
        const fetchedOrders = Array.isArray(data.allPlacedOrders) ? data.allPlacedOrders : data.order ? [data.order] : [];
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setOrders((prev) => prev.map((o) => o._id === orderId ? { ...o, status: newStatus as IOrder["status"] } : o));
      }
    } catch (err) {
  ...
  const processedOrders = useMemo(() => {
    ...
    if (searchQuery && searchQuery.length < 15) {
      filtered = filtered.filter((o) => o.user.email.toLowerCase().includes(searchQuery.toLowerCase()) || o._id.includes(searchQuery));
    }
    return filtered;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-8 flex flex-col gap-4 border-b border-green-100 pb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-green-900">Order Management</h1>
              <p className="text-green-600/70 text-sm font-medium">Tracking {processedOrders.length} orders</p>
            </div>
            ...
            >
              ...
              {["pending", "placed", "processing", "shipped", "delivered", "cancelled"].map((s) => (
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white border-2 border-dashed border-green-100 rounded-2xl">
            ...
            <h3 className="text-lg font-bold text-green-900">No Orders Found</h3>
            <button onClick={() => { setSearchQuery(""); setStatusFilter("all"); fetchOrders(); }} className="mt-4 text-green-600 font-bold hover:underline">
              Reset Filters
            </button>
          </div>
        ) : (
          <div className="space-y-6">
            {processedOrders.map((order) => (
              <div key={order._id} className="bg-white border border-green-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all">
                ...
                <div className="px-4 md:px-6 py-4 bg-slate-50/50 border-b border-green-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[11px] md:text-xs font-medium text-green-700">
                    <span className="flex items-center gap-1.5 bg-white px-2 py-1 rounded-md border border-green-100">
                      <Hash className="h-3.5 w-3.5 text-green-400" /> {order._id}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Clock className="h-3.5 w-3.5 text-green-400" /> {new Date(order.createdAt).toLocaleDateString()}
                    </span>
                  </div>
                  ...
                  <div className="relative inline-block w-full sm:w-auto">
                    ...
                    {updatingId === order._id && <Loader2 className="absolute -left-6 top-2 h-4 w-4 animate-spin text-green-600" />}
                  </div>
                </div>
                  ...
                  <div className="lg:col-span-5 space-y-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0">
                        <p className="text-base font-bold text-green-900 truncate">{order.user.name}</p>
                        <p className="text-sm text-green-600/70 font-medium truncate mb-2">{order.user.email}</p>
                        <span className={`inline-block px-2 py-0.5 text-[10px] font-bold border rounded-md uppercase ${userTypeStyles[order.user.userType]}`}>
                          {order.user.userType}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                      ...
                      <div className="text-sm text-slate-600 space-y-1">
                        <p className="font-semibold text-slate-800">{order.shippingAddress.street}</p>
                    ...
                    <div className="flex items-center justify-between p-4 bg-green-100 text-black rounded-2xl shadow-sm">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-white/10 rounded-lg"><CreditCard size={20} /></div>
                        <div>
                          <p className="text-[10px] font-bold uppercase opacity-60 tracking-wider">Total Payable</p>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs font-bold">{order.payment.payment_status.toUpperCase()}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="lg:col-span-7">
                    <div className="bg-slate-50/50 rounded-2xl border border-green-50 p-4 md:p-5">
                      <h4 className="text-[10px] font-bold text-green-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <Package size={14} /> Items Summary ({order.items.length})
                      </h4>
                      <div className="space-y-3">
                        {order.items.map((item, idx) => (
                          <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 bg-white rounded-xl border border-green-50">
